...
- **`inv-4phase-release-learning` — 4-Phase Release & Lean Learning Lifecycle**: Ecosystem delivery strictly follows the sequential progression: 1. Code & Local QA Gate $\rightarrow$ 2. User Mk1 Eyeball Review $\rightarrow$ 3. Feature Milestone Release (`vX.Y.0` via milestone branch PR & Code Owner review) $\rightarrow$ 4. `/learn` Retrospective $\rightarrow$ 5. Apply Lessons as Immediate Lean Patch Release (`vX.Y.1` directly on `main` upon human `learning_proposal.md` approval and local `just check` QA, eliminating redundant PR ceremony).
