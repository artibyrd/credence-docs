...
- **`inv-mk1-eyeball` — Human Review Gate ("Mk1 Eyeball") with Staged PR & Live Dev Links**: `git push` tags, PR merges, and prod deploys require explicit human Mk1 sign-off. Walkthroughs must link open PRs and live Dev endpoints after monitoring `deploy-dev.yml` before review presentation.
...
- **`inv-4phase-release-learning` — 4-Phase Release & Lean Learning Lifecycle**: 1. Code & QA (`just check`) $\rightarrow$ 2. Open PRs & Watch Dev Deploy (`deploy-dev.yml`) $\rightarrow$ 3. **User Mk1 Review (with PR & live Dev links)** $\rightarrow$ 4. Merge PRs, Tag Release (`vX.Y.0`), Watch Prod CI/CD $\rightarrow$ 5. `/learn` Retrospective $\rightarrow$ 6. Lean Patch (`vX.Y.1` via identical pipeline).
